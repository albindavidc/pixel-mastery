...
export function DomBomJsCurriculum() {
  ...
  const challenges = [
    {
      ...
      description: "Build a button that increments a counter displayed on the page each time it's clicked, using addEventListener and textContent.",
      requiredMethods: ['addEventListener', 'textContent'],
      verify: (c: string) => {
        const lower = c.toLowerCase();
        return lower.includes('addeventlistener') && lower.includes('textcontent');
      }
    },
    {
      title: "URL Reader",
      description: "Display the current window.location.href in a paragraph when a button is clicked.",
      requiredMethods: ['window.location', 'href'],
      verify: (c: string) => {
        const lower = c.toLowerCase();
        return (lower.includes('window.location') || lower.includes('location.href')) && lower.includes('href');
      }
    }
  ];
  ...
  const handleTryCode = (snippet: string) => {
    setCode(\`<!DOCTYPE html>\\n<html>\\n<body>\\n  <h2>API Test</h2>\\n  <div id="output" class="p-4 bg-zinc-100 rounded-lg text-zinc-800"></div>\\n  <script>\\n    // Generated Try It Snippet\\n    const output = document.getElementById('output');\\n    \${snippet}\\n  </script>\\n</body>\\n</html>\`);
    setActiveMode('try');
  };

  const useCases = [
    { title: "Dynamic Forms", desc: "Validating and updating form fields live as the user types, using querySelector and addEventListener." },
    { title: "Single-Page Navigation", desc: "Using window.history and window.location to change views without a full page reload." },
    { title: "Persisted State", desc: "Using localStorage or sessionStorage to remember user preferences or session data across visits." }
  ];
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="mb-6 h-[500px] flex flex-col">
        <CodeEditorPreview
          code={code}
          onChange={setCode}
   ...
   └── [ DOM: document ]  <-- The webpage itself
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden shadow-xl">
            ...
            <div className="p-5 bg-zinc-900/50 flex flex-col gap-3">
              {[
                { name: 'document', desc: 'The entry point to the webpage content.', snippet: 'console.log(document.title);' },
                { name: 'document.getElementById()', desc: 'Selects a specific element by ID.', snippet: "const el = document.createElement('div'); el.id = 'demo'; document.body.appendChild(el);\\nconsole.log(document.getElementById('demo'));" },
                { name: 'document.querySelector()', desc: 'Selects elements using CSS selectors.', snippet: "document.body.innerHTML += '<p class=\"test\">Hello!</p>';\\nconst el = document.querySelector('.test');\\noutput.textContent = el.textContent;" },
                { name: 'element.addEventListener()', desc: 'Attaches click, hover, or keyboard events to elements.', snippet: "const btn = document.createElement('button');\\nbtn.textContent = 'Click me';\\nbtn.className = 'px-4 py-2 bg-indigo-500 text-white rounded';\\ndocument.body.appendChild(btn);\\nbtn.addEventListener('click', () => alert('Clicked!'));" },
                { name: 'element.getAttribute()', desc: 'Reads HTML attributes (like href or src).', snippet: "const link = document.createElement('a');\\nlink.href = 'https://example.com';\\noutput.textContent = 'Link href: ' + link.getAttribute('href');" },
                { name: 'document.createElement()', desc: 'Generates a brand new HTML tag dynamically.', snippet: "const el = document.createElement('h3');\\nel.textContent = 'I am new here!';\\noutput.appendChild(el);" }
              ].map(item => (
                <div key={item.name} className="flex flex-col gap-1 border-b border-zinc-800/50 pb-3 last:border-0 last:pb-0">
                  <div className="flex items-center justify-between">
                    <code className="text-sky-400 text-sm font-mono w-fit">{item.name}</code>
                    <button onClick={() => handleTryCode(item.snippet)} className="flex items-center gap-1.5 text-xs font-semibold text-zinc-400 hover:text-white bg-zinc-800 hover:bg-zinc-700 px-2 py-1 rounded transition-colors">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden shadow-xl">
            ...
            <div className="p-5 bg-zinc-900/50 flex flex-col gap-3">
              {[
                { name: 'window', desc: 'The global object. All other BOM objects are properties of window.', snippet: "output.textContent = 'Window innerWidth: ' + window.innerWidth;" },
                { name: 'window.location', desc: 'Controls or reads the current URL.', snippet: "output.textContent = 'Current URL: ' + window.location.href;" },
                { name: 'window.history', desc: 'Navigates the user browser history.', snippet: "output.textContent = 'History length: ' + window.history.length;" },
                { name: 'window.navigator', desc: 'Contains information about the user browser, OS, and permissions.', snippet: "output.textContent = 'User Agent: ' + window.navigator.userAgent;" },
                { name: 'window.screen', desc: 'Gives properties about the user monitor screen resolution and orientation.', snippet: "output.textContent = 'Screen Width: ' + window.screen.width;" },
                { name: 'window.localStorage', desc: 'Saves data locally in the user browser.', snippet: "window.localStorage.setItem('demo', 'Hello from Storage');\\noutput.textContent = 'Saved value: ' + window.localStorage.getItem('demo');" },
                { name: 'alert() / confirm() / prompt()', desc: 'Triggers browser-native pop-up dialog boxes.', snippet: "if(confirm('Ready?')) {\\n  output.textContent = 'User is ready!';\\n}" }
              ].map(item => (
                <div key={item.name} className="flex flex-col gap-1 border-b border-zinc-800/50 pb-3 last:border-0 last:pb-0">
                  <div className="flex items-center justify-between">
                    <code className="text-emerald-400 text-sm font-mono w-fit">{item.name}</code>
                    <button onClick={() => handleTryCode(item.snippet)} className="flex items-center gap-1.5 text-xs font-semibold text-zinc-400 hover:text-white bg-zinc-800 hover:bg-zinc-700 px-2 py-1 rounded transition-colors">
                      <Play className="w-3 h-3" /> Try it
                    </button>
                  </div>
          ...
          <div className="max-w-4xl mx-auto flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div className="flex-1 w-full">
              ...
              <div className="space-y-3 mt-4 pt-4 border-t border-zinc-800/50">
                <div className="text-xs font-medium text-zinc-500 uppercase tracking-wider">Required Methods</div>
                <div className="flex flex-wrap gap-2">
                  {challenge.requiredMethods.map(target => {
                    const isMet = code.toLowerCase().includes(target.toLowerCase());
                    return (
                      <span
                        key={target}
      ...
      <div className="bg-zinc-900 border border-zinc-800/50 rounded-2xl p-6 mt-4 relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <h3 className="text-lg font-bold text-white tracking-tight">JavaScript Core Curriculum</h3>
        </div>
        ...
        <div className="flex flex-wrap gap-2 md:gap-3 items-center">
           <div className="bg-zinc-800/80 border border-zinc-700/50 px-4 py-2 rounded-xl flex items-center gap-2 opacity-50">
             ...
             <span className="text-sm font-medium text-zinc-300">JS Fundamentals</span>
           </div>
           ...
           <div className="bg-indigo-500/10 border border-indigo-500/30 px-4 py-2 rounded-xl flex items-center gap-2 shadow-[0_0_15px_rgba(99,102,241,0.1)]">
             ...
             <span className="text-sm font-bold text-indigo-300">DOM & BOM</span>
           </div>
...
console.log('Fixed DomBomJsCurriculum.tsx');
