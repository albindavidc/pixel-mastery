...
export function Playground() {
  ...
  const handlePropertyClick = (prop: string) => {
    const isWildcard = prop.endsWith('-*');
  ...
  return (
    <div className="flex-1 bg-zinc-950 flex flex-col h-full overflow-hidden relative transition-colors">
      ...
      <div className="flex-shrink-0 bg-zinc-900 border-b border-zinc-800 flex flex-col z-10 shadow-sm transition-colors w-full sticky top-0">
        <div className="flex items-center justify-between gap-4 p-3 border-b border-zinc-800/50">
          <div 
            className="flex-1 overflow-x-auto scrollbar-thin flex items-center gap-4 px-1 pb-1 min-w-0" 
            onWheel={handleWheel}
          >
            {activeControlData.map((group, gIdx) => (
              <div key={group.group} className="flex items-center gap-1.5 shrink-0">
                {gIdx > 0 && <div className="w-px h-4 bg-zinc-700/50 mx-2" />}
                {group.properties.map(prop => {
                  const isWildcard = prop.endsWith('-*');
                  const isSpecialWildcard = prop === 'flex' || prop === 'grid';
                  
                  // For wildcards, it's active if any of its variants are in the class string
                  // Or if it's the currently selected property
                  let isActive = false;
                  if (isWildcard) {
                    const variants = wildcards[prop] || [];
                    ...
                    // Optionally, if they are special wildcards, we can also check if they are selected
        ...
        {/* Row 2: Variants */}
        <div 
          className={`overflow-hidden transition-all duration-200 ease-out bg-zinc-900/50 ${selectedProperty ? 'max-h-20 opacity-100 border-b border-zinc-800/50' : 'max-h-0 opacity-0'}`}
        >
          <div 
            className="p-3 overflow-x-auto scrollbar-thin flex items-center gap-1.5 px-4 pb-2 min-w-0" 
            onWheel={handleWheel}
          >
