...
export function TextShadowPanel({ activeClassesSet, onVariantClick, onCustomApply, onCustomRemove }: PanelProps) {
  const [selectedColorFamily, setSelectedColorFamily] = useState<string | null>(null);

  const shadowSizes = ['text-shadow-2xs', 'text-shadow-xs', 'text-shadow-sm', 'text-shadow-md', 'text-shadow-lg', 'text-shadow-none'];
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center gap-1.5">
        <span className="text-[10px] font-bold text-zinc-500 uppercase w-full">SIZE</span>
        {shadowSizes.map(cls => (
          <button
            key={cls}
            onClick={() => onVariantClick(cls)}
        ...
        <CustomValueInput property="text-shadow" activeClassesSet={activeClassesSet} onApply={onCustomApply} onRemove={onCustomRemove} />
      </div>

      <div className="w-full h-px bg-zinc-800/50"></div>

      <div className="flex flex-wrap items-center gap-1.5">
        <span className="text-[10px] font-bold text-zinc-500 uppercase w-full">COLOR</span>
        {baseColors.map(c => {
          const cls = `text-shadow-${c}`;
          return (
            <button
              key={c}
              onClick={() => onVariantClick(cls)}
        ...
        {renderColorRange('text-shadow')}
        <div className="w-full h-px bg-zinc-800/50 my-1"></div>
        <CustomValueInput property="text-shadow-color" activeClassesSet={activeClassesSet} onApply={onCustomApply} onRemove={onCustomRemove} />
      </div>
    </div>
  );
}
