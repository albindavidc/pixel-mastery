...
const newQuickRef = `        {/* Quick Reference Guide */}
        <div className="mb-8 bg-zinc-900 border border-zinc-800 rounded-xl p-5 shadow-lg">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            ...
            {/* Legend */}
            <div className="flex items-center gap-4 text-[10px] font-bold tracking-widest uppercase bg-zinc-950 p-2 rounded-lg border border-zinc-800">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {['flex-direction', 'flex-wrap', 'flex'].map(prop => (
                    <span key={prop} className="px-2 py-1 bg-sky-500/10 text-sky-300 border border-sky-500/20 rounded text-[10px] font-mono">{prop}</span>
                  ))}
                </div>
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {['flex-basis', 'flex-grow', 'flex-shrink', 'order'].map(prop => (
                    <span key={prop} className="px-2 py-1 bg-fuchsia-500/10 text-fuchsia-300 border border-fuchsia-500/20 rounded text-[10px] font-mono">{prop}</span>
                  ))}
                </div>
              </div>
            </div>
